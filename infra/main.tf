...
resource "google_cloud_run_v2_job" "targeted_research" {
  ...
  template {
    ...
    # native "reject if already running" switch. In practice the only
    # scheduled trigger is the single daily Cloud Scheduler cron below, so
    # overlap would only happen via a manual concurrent execution; see
    # infra/README.md.
    parallelism = 1
    task_count  = 1
...
resource "google_cloud_scheduler_job" "daily_trigger" {
  ...
  attempt_deadline = "${min(var.task_timeout_seconds + 30, 1800)}s"
