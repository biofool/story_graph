# Talk-page update proposal: Peter Ralston (martial artist) — post-AfD recreation

Produced per `prompts/graph_to_wikipedia_update.md` from the story_graph
subgraph for Peter Ralston / Cheng Hsin. Target: a recreated article at
[Peter Ralston (martial artist)](https://en.wikipedia.org/wiki/Peter_Ralston_(martial_artist))
— proposed posting location: the draft's talk page
...
- **AfD history.** `Peter Ralston` was deleted at AfD in 2010 and again on
  **2026-09-15** ([2nd nomination](https://en.wikipedia.org/wiki/Wikipedia:Articles_for_deletion/Peter_Ralston_(2nd_nomination)),
  closed by Vanamonde93). Direct recreation in mainspace would be
  subject to speedy deletion (G4) unless it addresses the AfD's concerns;
  AfC review is the appropriate path.
- **COI.** The requester (kkron / Wikipedia user Biofuel) authored the
  deleted article and has personal connections to the subject's milieu
  (Ralston is the Cheng Hsin teacher of aikido colleague Richard Moon).
  `direct_edit` is unavailable; an uninvolved editor should review.

## 2. Why this draft is different — AfD point-by-point
...
I (Biofuel) authored the previous article on this subject, deleted at AfD
in September 2026, and I know practitioners in the subject's milieu.
Per WP:COI I am not recreating the article directly; this draft is
submitted for review by uninvolved editors.
...
- **WP:NOR** — no synthesis; the Enter-the-Dragon comparison is the
  source's own framing, attributed.
- **WP:NPOV / WP:BLP** — no superlatives in the article's voice; "the
  Asian martial arts world was stunned" appears only as attributed
  quotation; birth year omitted as underdetermined rather than guessed.
- **WP:RS** — notability rests on edited magazines; self-pub sources
  carry only uncontroversial detail; Wikipedia is never cited.
- **WP:EL** — single official-site external link; no promotional links.
- **Process** — AfC submission, not direct recreation (AfD history + COI).
